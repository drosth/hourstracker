...
object Application extends App
  ...
  val registrations: Registrations = registrationService.readRegistrationsFrom(fileName)
      .filter(RegistrationSelector.registrationsBetween(
  ...
  private val consolidatedRegistrations: Map[String, ConsolidatedRegistrations] =
    consolidatedRegistrationService.consolidateRegistrations(registrations).groupBy(_.job)


  private val consolidatedRegistrationsWithMissingEntries = consolidatedRegistrationService
      .addUnregisteredEntriesTo(consolidatedRegistrations)

  println(jsonPresenter.renderRegistrationsTo(consolidatedRegistrationsWithMissingEntries).prettyPrint)

  consolidatedRegistrationsWithMissingEntries.foreach {
    case (job, registrations) => {

      val fileName = s"[Timesheet] - $job - ${dateRangeAsStringOf(registrations)}"
