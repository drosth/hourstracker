...
object Registration {
  ...
  object JsonProtocol extends JsonSupport with JsonDateTimeSupport with JsonMapSupport {
    implicit lazy val registrationFormat: RootJsonFormat[Registration] = jsonFormat12(Registration.apply)
  }
}
...
case class Registration(job: String,
  ...
  def calculateDurationInMinutes(): Long = (clockedIn, clockedOut) match {
    case (Some(in: LocalDateTime), Some(out: LocalDateTime)) => ChronoUnit.MINUTES.between(in, out)
    case _ => 0L
  }
}
